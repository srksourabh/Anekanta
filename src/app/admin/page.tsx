...
export default function AdminPage() {
  ...
  if (loading) return <div className="max-w-4xl mx-auto p-8 text-center text-stone-500">{t('loading')}</div>;

  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-stone-900 mb-6">{t('admin_title')}</h1>

      <div className="flex gap-4 mb-6">
        <button onClick={() => setTab('overview')} className={`px-4 py-2 rounded-lg font-medium ${tab === 'overview' ? 'bg-teal-600 text-white' : 'bg-stone-100 text-stone-700'}`}>
          {t('admin_overview')}
        </button>
        <button onClick={() => setTab('moderation')} className={`px-4 py-2 rounded-lg font-medium ${tab === 'moderation' ? 'bg-teal-600 text-white' : 'bg-stone-100 text-stone-700'}`}>
        ...
        <button onClick={() => setTab('roles')} className={`px-4 py-2 rounded-lg font-medium ${tab === 'roles' ? 'bg-teal-600 text-white' : 'bg-stone-100 text-stone-700'}`}>
          {t('admin_roles_tab')}
        </button>
      </div>
      ...
      {tab === 'overview' && stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: t('admin_total_debates'), value: stats.totalDebates, color: 'bg-teal-50 text-teal-700 border-teal-200' },
            { label: t('admin_total_arguments'), value: stats.totalArguments, color: 'bg-stone-50 text-stone-700 border-stone-200' },
      ...
      {tab === 'moderation' && (
        <div className="space-y-4">
          {flagged.length === 0 ? (
            <p className="text-stone-500">{t('admin_no_flagged')}</p>
          ) : flagged.map(item => (
            <div key={item.id} className={`p-4 rounded-xl border ${item.status === 'pending' ? 'border-red-200 bg-red-50' : 'border-stone-200 bg-stone-50'}`}>
              <div className="flex justify-between items-start mb-2">
                <div>
                  <span className="text-xs font-medium uppercase bg-stone-200 text-stone-700 px-2 py-0.5 rounded">{item.content_type}</span>
                  <span className="ml-2 text-xs text-stone-500">{t('admin_score')} {(item.score ?? 0).toFixed(2)}</span>
              ...
              <p className="text-sm text-stone-700 mb-2">{item.reason}</p>
              {item.status === 'pending' && (
      ...
      {tab === 'roles' && (
        <div>
          <p className="text-sm text-stone-500 mb-4">{t('admin_roles_desc')}</p>
          <input
            type="text"
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-stone-200">
                    <th className="text-left py-3 px-3 text-stone-600 font-medium">{t('auth_display_name')}</th>
                    {GLOBAL_ROLES.map(role => (
                      <th key={role} className="text-center py-3 px-3 text-stone-600 font-medium">
                      ...
                      <tr key={user.id} className="border-b border-stone-100 hover:bg-stone-50/50">
                        <td className="py-3 px-3">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <span className="font-medium text-stone-800">{user.display_name}</span>
                              {user.role === 'admin' && (
                                <span className="ml-2 text-[10px] bg-teal-100 text-teal-700 px-1.5 py-0.5 rounded">admin</span>
                              )}
                            </div>
                          </div>
                        </td>
