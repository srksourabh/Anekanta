...
export async function DELETE() {
  ...
  await db.prepare(`DELETE FROM flagged_content WHERE flagged_by = ?`).run(user.id);
  ...
  await db.prepare(`DELETE FROM arguments WHERE author_id = ?`).run(user.id);
  await db.prepare(`DELETE FROM debates WHERE author_id = ?`).run(user.id);
